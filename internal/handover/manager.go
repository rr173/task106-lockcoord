...
func (m *Manager) cancelHandoverInternal(h *model.Handover, operator, reason string) error {
	...
	// The status change and its timeline record must commit together. If writing
	// the timeline fails the whole transition is rolled back so the handover
	// keeps its prior status instead of appearing cancelled with no audit trail.
	return m.storage.CancelHandoverAtomic(h.ID, now, r, operator)
}
